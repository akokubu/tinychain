...
class P2P {
  constructor(port = 5001, endpoints = [], chain, wallet) {
    ...
    this.endpoints = endpoints; // 隣接するPeerのエンドポイント
  ...
  initServerSocket(socket) {
    ...
    this.handleMessage(socket); // messageハンドラを登録
  ...
  initClient(ws, endpoint) {
    ...
    ws.on("open", function open() {
      self.handleMessage(ws); // メッセージハンドラを登録
      ...
      switch (packet.type) {
        ...
        case PacketTypes.Vote:
          try {
            ...
            if (
              self.chain.votes.length !==
              self.chain.store.validators().length - 1
            )
              break; // 投票率が100%かチェック
            if (!self.chain.tallyVotes(self.chain.votes)) {
              // 2/3以上の賛同を得られなければ、ブロックを作り直す
              self.chain.proposerBlock = null;
              self.chain.votes = [];
              break;
            }

            // 2/3以上の賛同を得られれば、ブロックにsignしてブロードキャスト
        ...
        case PacketTypes.Block:
          try {
            ...
            if (!isNew) break; // 新しいブロックでない場合は、ブロードキャスしない
        ...
        case PacketTypes.PBlock: {
          ...
          // 既に同じブロックに投票済みならスキップ
          if (
            self.chain.votes.find(
              (v) => v.blockHash === b.hash && v.voter === self.wallet.pubKey
            )
          ) {
            break;
          }
          ...
          // Voteを作ってブロードキャスト
          const v = self.wallet.signVote(
            new Vote(b.height, b.hash, self.wallet.pubKey, isYes)
          );
